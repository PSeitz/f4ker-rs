...
mod test {
    ...
    fn bench_random_from_arr(b: &mut test::Bencher) {
        ...
        b.iter(|| {
            thread_rng().choose(&ADJECTIVE).unwrap()
        })
    }
     #[bench]
    ...
    fn bench_interpol_many(b: &mut test::Bencher) {
        b.iter(|| {
            let facker = Faker::new();
            (0..1000).map(|_|facker.fake("{{name.last_name}}, {{name.first_name}} {{name.suffix}}")).collect::<Vec<_>>()
        })
    }
    #[bench]
    fn bench_interpol_many_2(b: &mut test::Bencher) {
        b.iter(|| {
            let facker = Faker::new();
            (0..1000).map(|_|facker.fake2("{{name.last_name}}, {{name.first_name}} {{name.suffix}}")).collect::<Vec<_>>()
        })
    }
    #[bench]
    fn bench_interpol_many_custom(b: &mut test::Bencher) {
        ...
        b.iter(|| {
            (0..1000).map(|_|format!("{}, {} {}", name.last_name(None), name.first_name(None), name.suffix() )).collect::<Vec<_>>()
        })
    }
    #[bench]
